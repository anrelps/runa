...
const Form = ({
  ...
  return (
    ...
    >
      <h3 className='mb-6 text-4xl font-bold'>{t('contact.formTitle')}</h3>

      <div className='mb-6'>
        <p className='mb-2 text-2xl'>{t('contact.nameIntro')}</p>
        <input
          type='text'
          placeholder={t('contact.namePlaceholder')}
          className='w-full border-b bg-transparent p-2 transition-colors duration-750 focus:outline-0'
          style={{ borderColor: 'rgba(255,255,255,0.12)', color: 'var(--color-text-primary)' }}
        />
      </div>

      <div className='mb-6'>
        <p className='mb-2 text-2xl'>{t('contact.representsIntro')}</p>
      ...
      <AnimatePresence>
        {selected === 'company' && (
          <motion.div
            ...
            className='mb-6'
          >
            <p className='mb-2 text-2xl'>{t('contact.companyIntro')}</p>
            <input
              type='text'
      ...
      <div className='mb-6'>
        <p className='mb-2 text-2xl'>{t('contact.messageIntro')}</p>
        <textarea
  ...
  return (
    <div className='flex w-fit gap-px overflow-hidden rounded-lg font-medium'>
      <button
        ...
        style={{ color: selected === 'individual' ? 'var(--color-background-primary)' : 'var(--color-text-primary)' }}
        onClick={() => setSelected('individual')}
      >
        <span className='relative z-10'>{t('contact.individual')}</span>
        {selected === 'individual' && (
          <motion.div
            transition={BASE_TRANSITION}
            layoutId='form-tab'
      ...
      <button
        type='button'
        className='relative cursor-pointer rounded-lg px-3 py-1.5 text-sm transition-colors duration-750'
        style={{ color: selected === 'company' ? 'var(--color-background-primary)' : 'var(--color-text-primary)' }}
        onClick={() => setSelected('company')}
      >
        <span className='relative z-10'>{t('contact.company')}</span>
        {selected === 'company' && (
          <motion.div
            transition={BASE_TRANSITION}
            layoutId='form-tab'
...
const Images = ({ selected }: { selected: 'company' | 'individual' }) => (
  <div className='relative min-h-25 w-full overflow-hidden' style={{ backgroundColor: 'var(--color-background-primary)' }}>
    <motion.div
      initial={false}
      ...
      style={{
        ...
        backgroundImage: 'url(https://images.unsplash.com/photo-1499951360447-b19be8fe80f5?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1740&q=80)',
    ...
    />
    <div className='pointer-events-none absolute inset-0' style={{ backgroundColor: 'rgba(11,18,18,0.45)' }} />
  </div>
);
